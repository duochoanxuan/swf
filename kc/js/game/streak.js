...
Game.checkStreak = function(gem) {
	...
	} else if (Game.gem != null && Game.gem.id !== gem.id && !Game.gem.inStreak) {	
		// --- ĐÂY LÀ PHẦN DI CHUYỂN SAI (KHÔNG TẠO RA STREAK) ---
		
		// Phát âm thanh lỗi từ sfx.js (đảm bảo file sfx.js đã được thêm case 'error')
		if (typeof Game.playSfx === 'function') {
			Game.playSfx('error');
		}
...
Game.onStreakRemoved = function(streak) {		// We continue after the streak disappeared
	...
	for (var column in streak) {
		...
		// Kiểm tra nếu ô tồn tại trước khi gọi fallStreak để tránh lỗi crash logic
		var targetCell = get('#tile' + firstYToFall + '_' + column);
		if (targetCell && typeof targetCell.fallStreak === 'function') {
			targetCell.fallStreak();
		}
	};
	Game.deselectGem();
};
...
Game.generateGems = function(x) {
	...
	for (var i = Game.GRID_SIZE - 1; i >= 0; i--) {
		...
		if (currentGem == null) {
			...
			if (check) value = 0;
			y = -1 * quantity;
			var gem = new Game.Gem(parseInt(x), y, value);
